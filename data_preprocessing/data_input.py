...
def slide_input(data, window_size, step_size, input_path, cleaning_type, filling_type, norm_data,
    ...
    for i in range(0, len(data.index.values) - window_size + 1, step_size):
        ...
        # Save the first and last timestamps of each sample.
        timestamp.append(['%d_%d.csv' % (station_id, i), tmp['TimeStamp'].iloc[0], tmp['TimeStamp'].iloc[-1]]) 
        print(i)
    return timestamp
...
def get_label_pred(station, eq_list, last_timestamp, class_type, predict_size):
    ...
    for i in range(len(last_timestamp)):
        ...
        if len(eq_sel.values) > 0:
            ...
            # Choose the earthquake with the smallest epicenter distance when multiple earthquakes have the same maximum magnitude.
...
def data_input(data, data_type, dataset_split_time, window_size, step_size, eq_list, sta_info, predict_size, input_path,
    ...
    # Add a new column to convert time in the earthquake catalog to timestamp.
    ...
    if train_phase == 'train':
        ...
        if len(test_data.index.values) > 0:
            ...
            # Obtain the midnight timestamp every Saturday and predict earthquakes for the next week.
            ...
            # Use the last timestamp of each sample for labeling.
